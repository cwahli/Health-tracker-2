...
- **Biomarker Card Registry Normalization & Debug Log Download Fix (2026-08-16):** Fixed missing extraction table caused by `agentCardRegistry` and `AGENT_REGISTRY` missing `agent1_step1` / `medical` `biomarker_table_view` entries; normalized `agentType` to `'agent1'` with raw step in `agentTypeStep` in `App.tsx`, `BiomarkerCard.tsx`, and `LogChat.tsx`. Fixed "Download Debug Logs" button doing nothing when `jobId` prop was not directly bound on `LogChat` by extracting `targetJobId` from `msg.id` / `msg.data.jobId`, and fixed R2 debug payload retrieval in `server.ts` and `src/utils/r2Storage.ts` to check user-scoped and anonymous cold debug keys (`debug/${userId}/${jobId}.json`). All 27 biomarker lifecycle unit tests, linting, and production build pass cleanly.
- **Medical Insights Page Consolidated Table & R2 Debug Payload Upload Fix (2026-08-16):** Restored the `Lab Parser` (`agent1`) workflow step in `InsightsTab.tsx` and integrated `AgentResultTable` inside the `latestExtraction` view so extracted lab biomarkers are rendered directly as an interactive, consolidated table on the Medical Insights page. Resolved the `[R2Storage] Failed uploading debug payload to R2: Failed to fetch` client error by fixing relative TypeScript module imports (`./src/utils/debugPayload` and `./src/utils/r2Storage`) across `server.ts`, `serverJobs.ts`, and `serverBugSnapshot.ts`. Verified with successful application build (`compile_applet`).
- **Review & Approve Modal Biomarker Extraction Flow Fix (2026-08-16):** Fixed issue where clicking "Review & Approve" in `InsightsTab.tsx` opened an empty `LogChat` modal. Replaced the empty chat popup with a dedicated full-screen `AgentResultTable` modal (`fullscreenExtractionAnalysis`) that displays all extracted biomarkers with their units, values, and reference ranges. Implemented `handleApproveExtractionAnalysis` to persist approved biomarkers to profile & biomarker history and mark `agent1` as approved. Also hid the pending approval badge when an extraction result contains 0 markers and is not in progress. Verified `tsc --noEmit` and production build pass with zero errors.
