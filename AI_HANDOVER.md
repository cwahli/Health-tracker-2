...
| **Q-8** Process goldens | **Truth:** `docs/agent/domains/debug-contract.md`. Execute: RELIABILITY §11 / F-8.13 / Q-8.1–8.6. | **Q-8.1–Q-8.5 COMPLETE**: food + biomarker + receptionist process boards (`tests/foodProcess.golden.test.ts`, `tests/bioProcess.golden.test.ts`, `tests/deskProcess.golden.test.ts`). Food ledger laws n/a on receptionist/medical packs. Next: Q-8.3 stubbed dialog inventory then one website Log Meal (Q-8.6). |
...
| **F-10** Meal Agent | F-10.1–F-10.7 shipped. F-10.6: TS fat/Na critic in `finalizeDishLedger` via `decidePrepAddition` (env × method). Residual named `prepAddition.reason` — not 90% on Case 4/9. | F-10.8 Soak (Grok reviews). |
