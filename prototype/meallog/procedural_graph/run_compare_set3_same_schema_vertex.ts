 ...
 * SAME SCHEMA A/B: isolates procedural-step framing from schema compactness.
 * Arm A: production scout_only_compare instruction
 * Arm B: procedural steps only
 * BOTH arms: scoutOnlyCompareResponseSchema (identical required fields / bbox / groups)
 * Blank user text. Same photos. Same model. Shared patient + anti-collapse.
 */
import fs from 'fs';
...
const OUT = path.join(process.cwd(), 'prototype', 'meallog', 'procedural_graph', 'compare_set3_same_schema_vertex_results.json');
const SCORE = path.join(process.cwd(), 'prototype', 'meallog', 'procedural_graph', 'compare_set3_same_schema_vertex_scorecard.md');
...
async function main() {
  ...
  console.log('Running Arm B (procedural steps, SAME production schema)...');
  const armB = await runArm('procedural_steps_same_schema', proceduralSystem, proceduralUser, scoutOnlyCompareResponseSchema as any);
  ...
  const payload = {
    ...
    fairness: {
      ...
      schemaDiffers: false,
      sameResponseSchema: 'scoutOnlyCompareResponseSchema',
      note: 'Both arms use production scoutOnlyCompareResponseSchema. Only system/user framing differs (production instruction vs procedural steps). Isolates graph/step benefit from schema compactness.',
    },
    groundTruthSet3: {
      targetDishCount: 104,
  ...
  const md = `# Same-schema Set 3 A/B (Vertex) — isolates graph steps vs schema — ${payload.testDate}
