...
- QUANTITY & MULTIPACKS: Output 'weightGrams' (consumed serving) and 'packGrams' (container total). For unopened grocery multi-packs (e.g. '5 x 65ml', 'pack of 6') without explicit user notes stating all N units were consumed, set 'weightGrams' to a single unit/serving size (e.g. 65g) and 'packGrams' to the container total (e.g. 325g). Never omit an unopened multi-pack and never estimate the whole container as consumed: weightGrams is ALWAYS one serving here — the portion question resolves the true amount.
...
- CLINICAL VERDICT & NARRATIVE: Provide a 3-6 word 'verdict' ('level': good|warning|alert|neutral) and a direct 35-70 word clinical 'clinicalAdvice' in 2nd person ("You got..."). Lead with the single most significant finding: any nutrient far over budget must be flagged plainly with its magnitude, worse when the 7-day average is already over — then state what that means for health and give one actionable next step/movement.
