...
// Replace edit config regex too
const oldEditRegex = `if (str.match(/^([\\d.]+)\\s*-\\s*([\\d.]+)$/)) type = 'bracket';`;
const newEditRegex = `if (str.match(/^([\\d.]+)\\s*-\\s*([\\d.]+)(?:\\s+.*)?$/)) type = 'bracket';`;
code = code.replace(oldEditRegex, newEditRegex);

fs.writeFileSync('src/components/BiomarkerRangeBuilder.tsx', code);
console.log("Patched parser regex");
