...
export function buildVisualScoutPrompt(message: string, imageCount: number): string {
  if (imageCount <= 0) {
    return `Analyze the user's message: "${message}" and extract all food items or products mentioned. For each item, estimate its weight in grams and emit realistic portion nutrients in "nutrients" (calories, protein, totalFat, saturatedFat, transFat, carbohydrates, sugar, addedSugar, totalFibre, sodium, potassium, omega3, calcium, iron, magnesium, vitaminD). If any item is from a known chain or brand (e.g. McDonald's, Yolk), capture exact brand and dish name in originalName.`;
  }
  return `Analyze the provided ${imageCount > 1 ? imageCount + ' images' : 'image'} and list the food items you see, taking into consideration the user's message: "${message}". For each distinct food item, estimate its weight in grams and emit realistic portion nutrients in "nutrients" (calories, protein, totalFat, saturatedFat, transFat, carbohydrates, sugar, addedSugar, totalFibre, sodium, potassium, omega3, calcium, iron, magnesium, vitaminD).${
    imageCount > 1
      ...
      : ''
  } If any identified dish is from a known chain or brand (e.g. McDonald's, Yolk, Starbucks), capture exact brand and dish name in originalName.`;
}
